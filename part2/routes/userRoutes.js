...
router.get('/ownedDogs', async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db.query(`SELECT dog.name, dog.dog_id FROM Dogs dog JOIN Users user ON dog.owner_id = user.user_id WHERE user.username = ?`, username);
    res.json(rows);
  }
  catch (error) {
    console.error('Couldnt get dogs from database');
    res.status(500).json({ error: 'Failed to get owners dogs from database' });
  }
});
