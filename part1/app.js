...
(async () => {
  ...
  app.get('/api/walkrequests/open', async (req, res) => {
    try {
      const [books] = await db.execute('SELECT req.request_id, dog.name AS dog_name, req.requested_time, req.duration_minutes, req.location, user.username AS owner_username FROM Dogs dog JOIN Users users ON dogs.owner_id = users.user_id');
      res.json(books);
    } catch (err) {
      res.status(500).json({ error: 'Failed to fetch dog list' });
    }
