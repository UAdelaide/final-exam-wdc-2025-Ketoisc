...
// Route to return summary of each walker as JSON
  app.get('/api/walkers/summary', async (req, res) => {
    try {
      const [books] = await db.execute("SELECT user.username AS walker_username, (SELECT COUNT(*) FROM WalkRatings) AS total_ratings, SELECT AVG(rating) AS average_rating, COUNT(DISTINCT ");
      res.json(books);
    } catch (err) {
      res.status(500).json({ error: 'Failed to fetch dog list' });
    }
